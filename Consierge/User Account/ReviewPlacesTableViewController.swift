//
//  ReviewPlacesTableViewController.swift
//  Concierge 0.1
//
...
class ReviewPlacesTableViewController: UITableViewController {
    ...
    func update() {
        ...
        //get all new restaurants, cafes, bars, and activities
        placesRequestTask = Task {
            if let places = try? await PlacesToReviewRequest().send(), places.count > 0 {
    ...
    func approveAPlace(_ place: Place, status: String) {
...
extension ReviewPlacesTableViewController: MassEditTableViewCellDelegate {
    ...
    func approveSelected() {
        ...
        if selectedRows == [] {
            ...
            for i in selectedRows {
                ...
                approveAPlace(place, status: "Approved")
            }
        }
    }
    ...
    func rejectSelected() {
        ...
        if selectedRows == [] {
            ...
            for i in selectedRows {
                let place = placesToReview[i.row]
                approveAPlace(place, status: "Rejected")
            }
        }
    }
}
